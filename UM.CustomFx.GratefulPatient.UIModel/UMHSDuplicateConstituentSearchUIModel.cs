...
namespace UM.CustomFx.GratefulPatient.UIModel
{
    ...
    public partial class UMHSDuplicateConstituentSearchUIModel
    {
        private Guid batchConstituentID = Guid.Empty;
        private Guid erbConstituentID = Guid.Empty;
        private Guid erbBatchRowID = Guid.Empty;
        ...
        {
            this.Loaded += UMHSDuplicateConstituentSearchUIModel_Loaded;
            this.EDITCONSTITUENTCONTEXT.ValueChanged += EDITCONSTITUENTCONTEXT_ValueChanged;
            this.CONSTITUENTID.ValueChanged += CONSTITUENTID_ValueChanged;
            this.SearchResultsLoaded += UMHSDuplicateConstituentSearchUIModel_SearchResultsLoaded;

            _save.InvokeAction += _save_InvokeAction;
            _cancel.InvokeAction += _cancel_InvokeAction;
        }

        private void CONSTITUENTID_ValueChanged(object sender, ValueChangedEventArgs e)
        {
            if (this.CONSTITUENTID.HasValue())
            { 
                this._customModel.LoadFromDataFormItem(this.ToDataFormItem());
                this.LoadFromBatch(this.CONSTITUENTID.Value);
        ...
        private void UMHSDuplicateConstituentSearchUIModel_SearchResultsLoaded(object sender, Blackbaud.AppFx.UIModeling.Core.SearchResultsLoadedEventArgs e)
        {
        ...
        private void SetRules()
        {
            try
            {
                ...
                //ValueListItem<DuplicateResolutionUIModel.SIMILARADDRESSCODES?> similaraddresscode = _customModel.SIMILARADDRESSCODE.DataSource[Conversions.ToInteger(dataFormLoadReply.DataFormItem.Values["SIMILARADDRESSCODE"].Value)];
                //_customModel.SIMILARADDRESSCODE.ResetDataSource();
                //_customModel.SIMILARADDRESSCODE.DataSource.Add(similaraddresscode);
        ...
        private void LoadFromBatch(string context)
        {
            erbConstituentID = new Guid(Strings.Left(context, 36));
            erbBatchRowID = new Guid(Strings.Right(context, 36));
            this.LoadFromBatch(erbBatchRowID);
        }
